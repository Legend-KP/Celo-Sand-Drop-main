...
/**
 * @title LivesRefresh
 * @notice Pay 0.05 USDT or 0.05 USDC to refresh lives on Celo Mainnet.
...
contract LivesRefresh {
    ...
    uint256 public constant FEE = 50_000; // 0.05 USDT/USDC (6 decimals)
    ...
    event LivesRefreshed(address indexed player, address indexed token, uint256 amount, uint256 timestamp);
    ...
    /**
     * @notice Pay 0.05 USDT to refresh lives.
    ...
    function payWithUSDT() external nonReentrant whenNotPaused {
        ...
        emit LivesRefreshed(msg.sender, USDT, FEE, block.timestamp);
    }

    /**
     * @notice Pay 0.05 USDC to refresh lives.
     * @dev Effects are recorded before the token transfer to preserve CEI.
     */
    function payWithUSDC() external nonReentrant whenNotPaused {
        ...
        emit LivesRefreshed(msg.sender, USDC, FEE, block.timestamp);
    }
